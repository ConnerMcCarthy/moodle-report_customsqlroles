 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class report_customsqlroles_report_testcase extends advanced_testcase {
    ...
    public function test_get_week_starts($startwday, $datestr, $currentweek, $lastweek) {
        ...
        set_config('startwday', $startwday, 'report_customsqlroles');

        $expected = [strtotime($currentweek), strtotime($lastweek)];
        $this->assertEquals($expected, report_customsqlroles_get_week_starts(strtotime($datestr)));
    }
    ...
    public function test_get_week_starts_use_calendar_default($startwday, $datestr, $currentweek, $lastweek) {
        ...
        set_config('startwday', -1, 'report_customsqlroles');
    ...
    public function test_get_month_starts_test() {
        $this->assertEquals(array(
                ...
                report_customsqlroles_get_month_starts(strtotime('12:36 10 November 2009')));

        $this->assertEquals(array(
                strtotime('00:00 1 November 2009'), strtotime('00:00 1 October 2009')),
                report_customsqlroles_get_month_starts(strtotime('00:00 1 November 2009')));

        $this->assertEquals(array(
                strtotime('00:00 1 November 2009'), strtotime('00:00 1 October 2009')),
                report_customsqlroles_get_month_starts(strtotime('23:59 29 November 2009')));
    }

    public function test_report_customsqlroles_get_element_type() {
        $this->assertEquals('date_time_selector', report_customsqlroles_get_element_type('start_date'));
        $this->assertEquals('date_time_selector', report_customsqlroles_get_element_type('startdate'));
        $this->assertEquals('date_time_selector', report_customsqlroles_get_element_type('date_closed'));
        $this->assertEquals('date_time_selector', report_customsqlroles_get_element_type('dateclosed'));

        $this->assertEquals('text', report_customsqlroles_get_element_type('anythingelse'));
        $this->assertEquals('text', report_customsqlroles_get_element_type('not_a_date_field'));
        $this->assertEquals('text', report_customsqlroles_get_element_type('mandated'));
    }

    public function test_report_customsqlroles_substitute_user_token() {
        $this->assertEquals('SELECT COUNT(*) FROM oh_quiz_attempts WHERE user = 123',
                report_customsqlroles_substitute_user_token('SELECT COUNT(*) FROM oh_quiz_attempts '.
                        'WHERE user = %%USERID%%', 123));
    }

    public function test_report_customsqlroles_capability_options() {
        $capoptions = array(
        ...
        $this->assertEquals($capoptions, report_customsqlroles_capability_options());

    }

    public function test_report_customsqlroles_runable_options() {
        $options = array('manual'  => get_string('manual', 'report_customsqlroles'),
        ...
        $this->assertEquals($options, report_customsqlroles_runable_options());
    }

    public function test_report_customsqlroles_daily_at_options() {
        $time = array();
        for ($h = 0; $h < 24; $h++) {
            $hour = ($h < 10) ? "0$h" : $h;
            $time[$h] = "$hour:00";
        }
        $this->assertEquals($time, report_customsqlroles_daily_at_options());
    }

    public function test_report_customsqlroles_email_options() {
        $options = array('emailnumberofrows' => get_string('emailnumberofrows', 'report_customsqlroles'),
                'emailresults' => get_string('emailresults', 'report_customsqlroles'));
        $this->assertEquals($options, report_customsqlroles_email_options());
    }

    public function test_report_customsqlroles_bad_words_list() {
        $options = array('ALTER', 'CREATE', 'DELETE', 'DROP', 'GRANT', 'INSERT', 'INTO', 'TRUNCATE', 'UPDATE');
        $this->assertEquals($options, report_customsqlroles_bad_words_list());
    }

    public function test_report_customsqlroles_contains_bad_word() {
        $string = 'DELETE * FROM prefix_user u WHERE u.id  > 0';
        $this->assertEquals(1, report_customsqlroles_contains_bad_word($string));
    }

    public function test_report_customsqlroles_get_ready_to_run_daily_reports() {
        global $DB;
        $this->resetAfterTest(true);
        ...
        list($today, $yesterday) = report_customsqlroles_get_daily_time_starts($timenow, $currenthour);
        ...
        $this->assertEquals(2, count(report_customsqlroles_get_ready_to_run_daily_reports($timenow)));
        ...
        list($oneam) = report_customsqlroles_get_daily_time_starts($timenow, 1);
        ...
        list($twoam) = report_customsqlroles_get_daily_time_starts($timenow, 2);
    ...
    public function test_report_customsqlroles_is_integer() {
        $this->assertTrue(report_customsqlroles_is_integer(1));
        $this->assertTrue(report_customsqlroles_is_integer('1'));
        $this->assertFalse(report_customsqlroles_is_integer('frog'));
        $this->assertFalse(report_customsqlroles_is_integer('2013-10-07'));
    }

    public function test_report_customsqlroles_get_table_headers() {
        $rawheaders = [
                'String date',
                'Date date',
                'URL to link',
        ...
        list($headers, $linkcolumns) = report_customsqlroles_get_table_headers($rawheaders);
    ...
    public function test_report_customsqlroles_pretify_column_names() {
        $row = new stdClass();
        $row->column = 1;
        $row->column_url = 2;
        $row->column_3 = 3;
        ...
        $this->assertEquals(['column', 'Column URL', 'column 3'],
                report_customsqlroles_pretify_column_names($row, $query));

    }

    public function test_report_customsqlroles_pretify_column_names_multi_line() {
        $row = new stdClass();
        $row->column = 1;
        $row->column_url = 2;
        $row->column_3 = 3;
    ...
    public function test_report_customsqlroles_pretify_column_names_same_name_diff_capitialisation() {
        $row = new stdClass();
        $row->course = 'B747-19B';
    ...
    public function test_report_customsqlroles_pretify_column_names_issue() {
        $row = new stdClass();
    ...
    public function test_report_customsqlroles_display_row() {
        $rawdata = [
                'Not a date',
        ...
        $this->assertEquals([
                ...
                '&lt;b&gt;Raw HTML&lt;/b&gt;'], report_customsqlroles_display_row($rawdata, $linkcolumns));
    }
     ...
     * @return void
     */
    public function test_report_customsqlroles_email_report() {
        global $CFG, $DB;
        ...
        $expectedsubject = get_string('emailsubjectnodata', 'report_customsqlroles',
        ...
        report_customsqlroles_email_report($report, $cvsfilename);
        $messages = $sink->get_messages();
    ...
    public function test_report_customsqlroles_write_csv_row() {
        global $CFG;
        $this->resetAfterTest();
        ...
        $handle = fopen($path, 'w');
        report_customsqlroles_write_csv_row($handle, ['"\\"', '","']);
