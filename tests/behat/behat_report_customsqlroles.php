 ...
 * All these steps include the phrase 'custom SQL report'.
 */
class behat_report_customsqlroles extends behat_base {
    ...
    public function the_following_custom_sql_report_exists(TableNode $data) {
        ...
        if (isset($report['category'])) {
            ...
            $report['categoryid'] = $this->get_category_id_by_name(
                    get_string('defaultcategory', 'report_customsqlroles'));
        }

        // Capability.
        if (isset($report['capability']) &&
                !in_array($report['capability'], report_customsqlroles_capability_options())) {
        ...
        if (isset($report['runable']) &&
                !in_array($report['runable'], report_customsqlroles_runable_options())) {
        ...
        if (!isset($report['timemodified'])) {
            $report['timemodified'] = \report_customsqlroles\utils::time();
        }
    ...
    protected function save_new_report(array $report) {
        ...
        $params = report_customsqlroles_get_query_placeholders_and_field_names($report['querysql']);
        if ($params) {
     ...
     * Views a report.
     *
     * Goes straight to the URL $CFG->wwwroot/report/customsqlroles/view.php?id=123
     *
    ...
    public function i_view_the_x_custom_sql_report(string $reportname) {
        ...
        $this->getSession()->visit($this->locate_path('/report/customsqlroles/view.php?id=' . $report->id));
    }
     ...
     *   | frogname | freddy |
     *   | colour   | green  |
     * this goes to the URL $CFG->wwwroot/report/customsqlroles/view.php?id=123&frogname=freddy&colour=green.
     *
    ...
    public function i_view_the_x_custom_sql_report_with_these_url_parameters(string $reportname, TableNode $data) {
        ...
        $this->getSession()->visit($this->locate_path('/report/customsqlroles/view.php?' .
                implode('&', $queryparams)));
    }

    /**
     * Sets a fake time for the report_customsqlroles
     *
    ...
    public function adhoc_database_queries_thinks_the_time_is($time) {
        ...
        set_config('behat_fixed_time', $value, 'report_customsqlroles');
    }
