@ou @ou_vle @report @report_customsqlroles
Feature: Ad-hoc database queries report
  ...
  Scenario: A query that uses the various auto-formatting options
    ...
    And "Not a date" row "String date" column of "report_customsqlroles_results" table should contain "Not a date"
    And "Not a date" row "Date date" column of "report_customsqlroles_results" table should contain "2018-11-22"
    And "Not a date" row "URL to link" column of "report_customsqlroles_results" table should contain "http://example.com/1"
    And "Not a date" row "Link text" column of "report_customsqlroles_results" table should contain "This is a link"
    And "Not a date" row "Not link" column of "report_customsqlroles_results" table should contain "Non-link, invalid URL"
    And "Not a date" row "Just a link url" column of "report_customsqlroles_results" table should contain "http://example.com/3"
    And "Not a date" row "HTML should be escaped" column of "report_customsqlroles_results" table should contain "<b>Raw HTML</b>"
    And "http://example.com/1" "link" should exist in the "report_customsqlroles_results" "table"
    And "This is a link" "link" should exist in the "report_customsqlroles_results" "table"
    And "Non-link, invalid URL" "link" should not exist in the "report_customsqlroles_results" "table"
    And "http://example.com/3" "link" should exist in the "report_customsqlroles_results" "table"
    And I should not see "Link text link url" in the "report_customsqlroles_results" "table"
    And I should see "This report has 1 rows."
  ...
  Scenario: Test reporting when a query exceeds the limit
    Given the following config values are set as admin:
      | querylimitdefault | 1 | report_customsqlroles |
    When I log in as "admin"
    And I navigate to "Reports > Ad-hoc database queries" in site administration
    And I press "Add a new query"
    And I set the following fields to these values:
  ...
  Scenario: View an Ad-hoc database query that returns data that confuses PHP CSV parsing
    ...
    When I log in as "admin"
    And I view the "Test query" custom sql report
    Then "\" row "Comma" column of "report_customsqlroles_results" table should contain ","
