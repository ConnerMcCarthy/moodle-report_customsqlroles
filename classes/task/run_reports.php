...
namespace report_customsqlroles\task;
...
class run_reports extends \core\task\scheduled_task {
    ...
    public function get_name() {
        return get_string('crontask', 'report_customsqlroles');
    }
    ...
    public function execute() {
        ...
        list($startofthisweek, $startoflastweek) = report_customsqlroles_get_week_starts($timenow);
        list($startofthismonth) = report_customsqlroles_get_month_starts($timenow);

        mtrace("... Looking for old temp CSV files to delete.");
        $numdeleted = report_customsqlroles_delete_old_temp_files($startoflastweek);
        if ($numdeleted) {
        ...
        foreach ($reportstorun as $report) {
            mtrace("... Running report " . report_customsqlroles_plain_text_report_name($report));
            try {
                report_customsqlroles_generate_csv($report, $timenow);
            } catch (\Exception $e) {
