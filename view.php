...
// Setup the page.
admin_externalpage_setup('report_customsqlroles', '', $urlparams,
        '/report/customsqlroles/view.php', ['pagelayout' => 'report']);
...
report_customsqlroles_log_view($id);

if ($report->runable == 'manual') {

    // Allow query parameters to be entered.
    if (!empty($report->queryparams)) {
        $queryparams = report_customsqlroles_get_query_placeholders_and_field_names($report->querysql);
        ...
        $mform = new report_customsqlroles_view_form(report_customsqlroles_url($relativeurl), $queryparams);
        $formdefaults = [];
        ...
        if (($newreport = $mform->get_data()) || count($paramvalues) == count($queryparams)) {
            ...
            admin_externalpage_setup('report_customsqlroles', '', $urlparams,
                    '/report/customsqlroles/view.php');
            $PAGE->set_title(format_string($report->displayname));
            $PAGE->navbar->add(format_string($report->displayname));
            echo $OUTPUT->header();
    ...
    if ($csvtimestamp === null) {
        $archivetimes = report_customsqlroles_get_archive_times($report);
...
if (!empty($paramvalues)) {
    foreach ($paramvalues as $name => $value) {
        ...
        echo html_writer::tag('p', get_string('parametervalue', 'report_customsqlroles',
...
if (is_null($csvtimestamp)) {
    echo html_writer::tag('p', get_string('nodatareturned', 'report_customsqlroles'));
} else {
    list($csvfilename, $csvtimestamp) = report_customsqlroles_csv_filename($report, $csvtimestamp);
    if (!is_readable($csvfilename)) {
        echo html_writer::tag('p', get_string('notrunyet', 'report_customsqlroles'));
    } else {
        $handle = fopen($csvfilename, 'r');

        if ($report->runable != 'manual' && !$report->singlerow) {
            echo $OUTPUT->heading(get_string('reportfor', 'report_customsqlroles',
                    userdate($csvtimestamp, get_string('strftimedate'))), 3);
        }

        $table = new html_table();
        $table->id = 'report_customsqlroles_results';
        list($table->head, $linkcolumns) = report_customsqlroles_get_table_headers(
                report_customsqlroles_read_csv_row($handle));
        ...
        if ($rowlimitexceeded) {
            echo html_writer::tag('p', get_string('recordlimitreached', 'report_customsqlroles',
                    $report->querylimit ?? get_config('report_customsqlroles', 'querylimitdefault')),
                    array('class' => 'admin_note'));
        } else {
            echo html_writer::tag('p', get_string('recordcount', 'report_customsqlroles', $count),
                    array('class' => 'admin_note'));
        }

        echo report_customsqlroles_time_note($report, 'p');

        echo $OUTPUT->download_dataformat_selector(get_string('downloadthisreportas', 'report_customsqlroles'),
            new moodle_url(report_customsqlroles_url('download.php')), 'dataformat', ['id' => $id, 'timestamp' => $csvtimestamp]);

        $archivetimes = report_customsqlroles_get_archive_times($report);
        if (count($archivetimes) > 1) {
            echo $OUTPUT->heading(get_string('archivedversions', 'report_customsqlroles'), 3).
            ...
            foreach ($archivetimes as $time) {
                ...
                if ($time == $csvtimestamp) {
                    ...
                    echo html_writer::tag('a', $formattedtime,
                                array('href' => new moodle_url(report_customsqlroles_url('view.php'),
...
if (!empty($queryparams)) {
    echo html_writer::tag('p', html_writer::link(
            new moodle_url(report_customsqlroles_url('view.php'), array('id' => $id)),
            get_string('changetheparameters', 'report_customsqlroles')));
}

if (has_capability('report/customsqlroles:definequeries', $context)) {
    $imgedit = $OUTPUT->pix_icon('t/edit', get_string('edit'));
    $imgdelete = $OUTPUT->pix_icon('t/delete', get_string('delete'));
    echo html_writer::start_tag('p').
         $OUTPUT->action_link(new moodle_url(report_customsqlroles_url('edit.php'),
                 array('id' => $id)), $imgedit.' '.
                 get_string('editreportx', 'report_customsqlroles', format_string($report->displayname))).
         html_writer::end_tag('p').
         html_writer::start_tag('p').
         $OUTPUT->action_link(new moodle_url(report_customsqlroles_url('delete.php'), ['id' => $id]),
                 $imgdelete . ' ' . get_string('deletereportx', 'report_customsqlroles', format_string($report->displayname))) .
         html_writer::end_tag('p');
}

$imglarrow = $OUTPUT->pix_icon('t/left', '');
echo html_writer::start_tag('p').
     $OUTPUT->action_link(new moodle_url(report_customsqlroles_url('index.php')), $imglarrow.
             get_string('backtoreportlist', 'report_customsqlroles')).
     html_writer::end_tag('p').
     $OUTPUT->footer();
