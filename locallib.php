...
function report_customsqlroles_execute_query($sql, $params = null, $limitnum = null) {
    ...
    if ($limitnum === null) {
        $limitnum = get_config('report_customsqlroles', 'querylimitdefault');
    }
...
function report_customsqlroles_prepare_sql($report, $timenow) {
    global $USER;
    $sql = $report->querysql;
    if ($report->runable != 'manual') {
        list($end, $start) = report_customsqlroles_get_starts($report, $timenow);
        $sql = report_customsqlroles_substitute_time_tokens($sql, $start, $end);
    }
    $sql = report_customsqlroles_substitute_user_token($sql, $USER->id);
    return $sql;
}
 ...
 */
function report_customsqlroles_get_query_placeholders($sql) {
 ...
 */
function report_customsqlroles_get_query_placeholders_and_field_names(string $querysql): array {
    $queryparams = [];
    foreach (report_customsqlroles_get_query_placeholders($querysql) as $queryparam) {
 ...
 */
function report_customsqlroles_get_element_type($name) {
    $regex = '/^date|date$/';
...
function report_customsqlroles_generate_csv($report, $timenow) {
    global $DB;
    $starttime = microtime(true);

    $sql = report_customsqlroles_prepare_sql($report, $timenow);

    $queryparams = !empty($report->queryparams) ? unserialize($report->queryparams) : array();
    $querylimit  = $report->querylimit ?? get_config('report_customsqlroles', 'querylimitdefault');
    // Query one extra row, so we can tell if we hit the limit.
    $rs = report_customsqlroles_execute_query($sql, $queryparams, $querylimit + 1);
    ...
    foreach ($rs as $row) {
        if (!$csvtimestamp) {
            list($csvfilename, $csvtimestamp) = report_customsqlroles_csv_filename($report, $timenow);
            $csvfilenames[] = $csvfilename;

            if (!file_exists($csvfilename)) {
                $handle = fopen($csvfilename, 'w');
                report_customsqlroles_start_csv($handle, $row, $report);
            } else {
        ...
        foreach ($data as $name => $value) {
            if (report_customsqlroles_get_element_type($name) == 'date_time_selector' &&
                    report_customsqlroles_is_integer($value) && $value > 0) {
        ...
        report_customsqlroles_write_csv_row($handle, $data);
        $count += 1;
    }
    $rs->close();
        ...
        if ($csvfilenames) {
            foreach ($csvfilenames as $csvfilename) {
                if (!empty($report->emailto)) {
                    report_customsqlroles_email_report($report, $csvfilename);
                }
                if (!empty($report->customdir)) {
                    report_customsqlroles_copy_csv_to_customdir($report, $timenow, $csvfilename);
                }
            }
        } else { // If there is no data.
            if (!empty($report->emailto)) {
                report_customsqlroles_email_report($report);
            }
            if (!empty($report->customdir)) {
                report_customsqlroles_copy_csv_to_customdir($report, $timenow);
            }
        }
    }
    return $csvtimestamp;
 ...
 */
function report_customsqlroles_is_integer($value) {
    return (string) (int) $value === (string) $value;
}

function report_customsqlroles_csv_filename($report, $timenow) {
    if ($report->runable == 'manual') {
        return report_customsqlroles_temp_cvs_name($report->id, $timenow);

    } else if ($report->singlerow) {
        return report_customsqlroles_accumulating_cvs_name($report->id);

    } else {
        list($timestart) = report_customsqlroles_get_starts($report, $timenow);
        return report_customsqlroles_scheduled_cvs_name($report->id, $timestart);
    }
}

function report_customsqlroles_temp_cvs_name($reportid, $timestamp) {
    global $CFG;
    $path = 'admin_report_customsqlroles/temp/'.$reportid;
    make_upload_directory($path);
    return array($CFG->dataroot.'/'.$path.'/'.strftime('%Y%m%d-%H%M%S', $timestamp).'.csv',
                 $timestamp);
}

function report_customsqlroles_scheduled_cvs_name($reportid, $timestart) {
    global $CFG;
    $path = 'admin_report_customsqlroles/'.$reportid;
    make_upload_directory($path);
    return array($CFG->dataroot.'/'.$path.'/'.strftime('%Y%m%d-%H%M%S', $timestart).'.csv',
                 $timestart);
}

function report_customsqlroles_accumulating_cvs_name($reportid) {
    global $CFG;
    $path = 'admin_report_customsqlroles/'.$reportid;
    make_upload_directory($path);
...
function report_customsqlroles_get_archive_times($report) {
    global $CFG;
    if ($report->runable == 'manual' || $report->singlerow) {
        return array();
    }
    $files = glob($CFG->dataroot.'/admin_report_customsqlroles/'.$report->id.'/*.csv');
    $archivetimes = array();
...
function report_customsqlroles_substitute_time_tokens($sql, $start, $end) {
    return str_replace(array('%%STARTTIME%%', '%%ENDTIME%%'), array($start, $end), $sql);
}

function report_customsqlroles_substitute_user_token($sql, $userid) {
    return str_replace('%%USERID%%', $userid, $sql);
}

function report_customsqlroles_url($relativeurl) {
    global $CFG;
    return $CFG->wwwroot.'/report/customsqlroles/'.$relativeurl;
}

function report_customsqlroles_capability_options() {
    return array(
        'report/customsqlroles:view' => get_string('anyonewhocanveiwthisreport', 'report_customsqlroles'),
        'moodle/site:viewreports' => get_string('userswhocanviewsitereports', 'report_customsqlroles'),
        'moodle/site:config' => get_string('userswhocanconfig', 'report_customsqlroles')
    );
}

function report_customsqlroles_role_options() {
    $roles = role_get_names(context_system::instance(), ROLENAME_ALIAS, true);
    $options = array();
    foreach ($roles as $roleid => $label) {
        $options[(int)$roleid] = (string)$label;
    }
    return $options;
}

function report_customsqlroles_parse_allowed_role_ids($allowedroleids) {
    if (empty($allowedroleids)) {
        return array();
    }
    if (is_array($allowedroleids)) {
        $roleids = array_map('intval', $allowedroleids);
        return array_values(array_filter(array_unique($roleids)));
    }
    $roleids = preg_split('/\s*,\s*/', trim($allowedroleids), -1, PREG_SPLIT_NO_EMPTY);
    $roleids = array_map('intval', $roleids);
    return array_values(array_filter(array_unique($roleids)));
}

function report_customsqlroles_user_can_access_query($report, $context, $userid = null) {
    global $USER;
    ...
    if (has_capability('report/customsqlroles:definequeries', $context, $userid)) {
        return true;
    }

    if (!empty($report->capability) && !has_capability($report->capability, $context, $userid)) {
        return false;
    }

    $allowedroleids = report_customsqlroles_parse_allowed_role_ids(empty($report->allowedroleids) ? '' : $report->allowedroleids);
    if (!$allowedroleids) {
        return true;
    }
    ...
    foreach ($roles as $roleassignment) {
        if (in_array((int)$roleassignment->roleid, $allowedroleids, true)) {
            return true;
        }
    }
...
function report_customsqlroles_runable_options($type = null) {
    if ($type === 'manual') {
        return array('manual' => get_string('manual', 'report_customsqlroles'));
    }
    return array('manual' => get_string('manual', 'report_customsqlroles'),
                 'daily' => get_string('automaticallydaily', 'report_customsqlroles'),
                 'weekly' => get_string('automaticallyweekly', 'report_customsqlroles'),
                 'monthly' => get_string('automaticallymonthly', 'report_customsqlroles')
    );
}

function report_customsqlroles_daily_at_options() {
    $time = array();
    for ($h = 0; $h < 24; $h++) {
        $hour = ($h < 10) ? "0$h" : $h;
        $time[$h] = "$hour:00";
...
function report_customsqlroles_email_options() {
    return array('emailnumberofrows' => get_string('emailnumberofrows', 'report_customsqlroles'),
            'emailresults' => get_string('emailresults', 'report_customsqlroles'),
    );
}

function report_customsqlroles_bad_words_list() {
    return array('ALTER', 'CREATE', 'DELETE', 'DROP', 'GRANT', 'INSERT', 'INTO',
                 'TRUNCATE', 'UPDATE');
}

function report_customsqlroles_contains_bad_word($string) {
    return preg_match('/\b('.implode('|', report_customsqlroles_bad_words_list()).')\b/i', $string);
}

function report_customsqlroles_log_delete($id) {
    $event = \report_customsqlroles\event\query_deleted::create(
            array('objectid' => $id, 'context' => context_system::instance()));
    $event->trigger();
}

function report_customsqlroles_log_edit($id) {
    $event = \report_customsqlroles\event\query_edited::create(
            array('objectid' => $id, 'context' => context_system::instance()));
    $event->trigger();
}

function report_customsqlroles_log_view($id) {
    $event = \report_customsqlroles\event\query_viewed::create(
            array('objectid' => $id, 'context' => context_system::instance()));
    $event->trigger();
}
 ...
 * @param string $type, type of report (manual, daily, weekly or monthly)
 */
function report_customsqlroles_print_reports_for($reports, $type) {
    global $OUTPUT;
    ...
    if (!empty($type)) {
        $help = html_writer::tag('span', $OUTPUT->help_icon($type . 'header', 'report_customsqlroles'));
        echo $OUTPUT->heading(get_string($type . 'header', 'report_customsqlroles') . $help, 3);
    }

    $context = context_system::instance();
    $canedit = has_capability('report/customsqlroles:definequeries', $context);
    $capabilities = report_customsqlroles_capability_options();
    foreach ($reports as $report) {
        ...
        echo html_writer::tag('a', format_string($report->displayname),
                              array('href' => report_customsqlroles_url('view.php?id='.$report->id))).
             ' '.report_customsqlroles_time_note($report, 'span');
        if ($canedit) {
            $imgedit = $OUTPUT->pix_icon('t/edit', get_string('edit'));
            $imgdelete = $OUTPUT->pix_icon('t/delete', get_string('delete'));
            echo ' '.html_writer::tag('span', get_string('availableto', 'report_customsqlroles',
                                      $capabilities[$report->capability]),
                                      array('class' => 'admin_note')).' '.
                 html_writer::tag('a', $imgedit,
                         ['title' => get_string('editreportx', 'report_customsqlroles', format_string($report->displayname)),
                          'href' => report_customsqlroles_url('edit.php?id='.$report->id)]) . ' ' .
                 html_writer::tag('a', $imgdelete,
                            array('title' => get_string('deletereportx', 'report_customsqlroles', format_string($report->displayname)),
                                  'href' => report_customsqlroles_url('delete.php?id='.$report->id)));
        }
 ...
 */
function report_customsqlroles_get_table_headers($row) {
 ...
 */
function report_customsqlroles_display_row($row, $linkcolumns) {
    $rowdata = array();
...
function report_customsqlroles_time_note($report, $tag) {
    if ($report->lastrun) {
        $a = new stdClass;
        $a->lastrun = userdate($report->lastrun);
        $a->lastexecutiontime = $report->lastexecutiontime / 1000;
        $note = get_string('lastexecuted', 'report_customsqlroles', $a);

    } else {
        $note = get_string('notrunyet', 'report_customsqlroles');
    }
...
function report_customsqlroles_pretify_column_names($row, $querysql) {
    $colnames = [];
 ...
 */
function report_customsqlroles_write_csv_row($handle, $data) {
    global $CFG;
    $escapeddata = array();
 ...
 */
function report_customsqlroles_read_csv_row($handle) {
...
function report_customsqlroles_start_csv($handle, $firstrow, $report) {
    $colnames = report_customsqlroles_pretify_column_names($firstrow, $report->querysql);
    if ($report->singlerow) {
        array_unshift($colnames, get_string('queryrundate', 'report_customsqlroles'));
    }
    report_customsqlroles_write_csv_row($handle, $colnames);
}
 ...
 */
function report_customsqlroles_get_daily_time_starts($timenow, $at) {
    $hours = $at;
    $minutes = 0;
...
function report_customsqlroles_get_week_starts($timenow) {
    $dateparts = getdate($timenow);

    // Get configured start of week value. If -1 then use the value from the site calendar.
    $startofweek = get_config('report_customsqlroles', 'startwday');
    if ($startofweek == -1) {
...
function report_customsqlroles_get_month_starts($timenow) {
    $dateparts = getdate($timenow);
...
function report_customsqlroles_get_starts($report, $timenow) {
    switch ($report->runable) {
        case 'daily':
            return report_customsqlroles_get_daily_time_starts($timenow, $report->at);
        case 'weekly':
            return report_customsqlroles_get_week_starts($timenow);
        case 'monthly':
            return report_customsqlroles_get_month_starts($timenow);
        default:
            throw new Exception('unexpected $report->runable.');
    }
}

function report_customsqlroles_delete_old_temp_files($upto) {
    global $CFG;

    $count = 0;
    $comparison = strftime('%Y%m%d-%H%M%S', $upto).'csv';

    $files = glob($CFG->dataroot.'/admin_report_customsqlroles/temp/*/*.csv');
    if (empty($files)) {
        return;
    }
    foreach ($files as $file) {
 ...
 */
function report_customsqlroles_validate_users($userids, $capability) {
    global $DB;
    if (empty($userstring)) {
        return null;
    }
    ...
    $a->whocanaccess = get_string('whocanaccess', 'report_customsqlroles');

    foreach ($userids as $userid) {
        // Cannot find the user in the database.
        if (!$user = $DB->get_record('user', ['id' => $userid])) {
            return get_string('usernotfound', 'report_customsqlroles', $userid);
        }
        ...
        if (!has_capability($capability, $context, $user)) {
            return get_string('userhasnothiscapability', 'report_customsqlroles', $a);
        }
    }
    return null;
}

function report_customsqlroles_get_message_no_data($report) {
    // Construct subject.
    $subject = get_string('emailsubjectnodata', 'report_customsqlroles',
            report_customsqlroles_plain_text_report_name($report));
    $url = new moodle_url('/report/customsqlroles/view.php', array('id' => $report->id));
    $link = get_string('emailink', 'report_customsqlroles', html_writer::tag('a', $url, array('href' => $url)));
    $fullmessage = html_writer::tag('p', get_string('nodatareturned', 'report_customsqlroles') . ' ' . $link);
...
function report_customsqlroles_get_message($report, $csvfilename) {
    $handle = fopen($csvfilename, 'r');
    $table = new html_table();
    $table->head = report_customsqlroles_read_csv_row($handle);
    $countrows = 0;
    ...
    if ($countrows == 0) {
        ...
        $subject = get_string('emailsubject1row', 'report_customsqlroles',
                report_customsqlroles_plain_text_report_name($report));
    } else {
        $subject = get_string('emailsubjectxrows', 'report_customsqlroles',
                ['name' => report_customsqlroles_plain_text_report_name($report), 'rows' => $countrows]);
    }
    ...
    if ($countrows === 1) {
        $returnrows = html_writer::tag('span', get_string('emailrow', 'report_customsqlroles', $countrows));
    } else {
        $returnrows = html_writer::tag('span', get_string('emailrows', 'report_customsqlroles', $countrows));
    }
    $url = new moodle_url('/report/customsqlroles/view.php', array('id' => $report->id));
    $link = get_string('emailink', 'report_customsqlroles', html_writer::tag('a', $url, array('href' => $url)));
...
function report_customsqlroles_email_report($report, $csvfilename = null) {
    global $DB;
    ...
    // Get the message.
    if ($csvfilename) {
        $message = report_customsqlroles_get_message($report, $csvfilename);
    } else {
        $message = report_customsqlroles_get_message_no_data($report);
    }
    ...
    foreach ($userids as $userid) {
        ...
        $messageid = report_customsqlroles_send_email_notification($recipient, $message);
        if (!$messageid) {
            mtrace(get_string('emailsentfailed', 'report_customsqlroles', fullname($recipient)));
        }
    }
}
...
function report_customsqlroles_get_ready_to_run_daily_reports($timenow) {
    ...
    foreach ($reports as $id => $r) {
        ...
        if (!report_customsqlroles_is_daily_report_ready($r, $timenow)) {
            continue;
        }
        $reportstorun[$id] = $r;
    }
 ...
 */
function report_customsqlroles_send_email_notification($recipient, $message) {

    // Prepare the message.
    $eventdata = new \core\message\message();
    $eventdata->component         = 'report_customsqlroles';
 ...
 * @return boolean
 */
function report_customsqlroles_is_daily_report_ready($report, $timenow) {
    // Time when the report should run today.
    list($runtimetoday) = report_customsqlroles_get_daily_time_starts($timenow, $report->at);

    // Values used to check whether the report has already run today.
    list($today) = report_customsqlroles_get_daily_time_starts($timenow, 0);
    list($lastrunday) = report_customsqlroles_get_daily_time_starts($report->lastrun, 0);
 ...
 */
function report_customsqlroles_copy_csv_to_customdir($report, $timenow, $csvfilename = null) {
 ...
 */
function report_customsqlroles_plain_text_report_name($report): string {
